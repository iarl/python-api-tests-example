...
class PetController(BaseController):
    ...
    def add_new(self, body):
        return self.request.post(f'{self.base_url}/{self.endpoint}', body, self.headers)
    ...
    def find_by_status(self, status):
        return self.request.get(f'{self.base_url}/{self.endpoint}/findByStatus?status={status}')
